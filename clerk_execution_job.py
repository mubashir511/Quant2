"""Unattended entry point for the FTMO Execution Clerk check — the
"clerk/executioner" half of the boardroom architecture (see
ai/clerk_execution.py's own module docstring for the full design).

Invoked by a Windows Scheduled Task (still named "Quant2CopilotExecution"
in Task Scheduler as of this file's rename 2026-08-30 — renaming the
Task's own Action to point at this file's new path is a manual,
out-of-repo step; nothing in this repo does it automatically), polling
...
ai.clerk_execution.is_execution_due). Exits almost immediately, with
no MT5 connection attempt at all, on every poll that isn't currently due
— the actual check only runs once per
config.CLERK_EXECUTION_CHECK_INTERVAL_MINUTES-sized window (default
...
ai/clerk_execution.py (EXECUTION_LOCK_PATH/EXECUTION_LOCK_STALE_AFTER_
...
calling run_clerk_execution_check() too, so the two independent call
sites can never run that function concurrently (a real race found on
audit — see ai.clerk_execution's own comment next to those constants).
"""
...
logger = logging.getLogger("clerk_execution_job")
...
def main() -> None:
    ...
    # run_clerk_execution_check itself, so this toggle is respected
    ...
    try:
        ...
        try:
            ...
            if result is _TIMEOUT_SENTINEL:
                logger.error(
                    "Execution Clerk check timed out after %.0f minutes",
                    config.CLERK_EXECUTION_RUN_TIMEOUT_SECONDS / 60,
                )
        except Exception:
            logger.exception("Execution Clerk check failed")
